...
## A test asserting on a rendered duration is asserting on its own runtime

`test_doctor_fails_when_the_drain_has_given_up_though_ingest_looks_healthy` asserted `"last enqueued 0s ago"`. The age behind that string is `now - created_ts`, a plain elapsed difference, and `_fmt_age` truncates it with `int()`. Nothing in the assertion named a duration, so nothing suggested it was one: what it actually asserted was that the test's setup plus everything `lh doctor` does before it reaches the age all finish inside one second.
