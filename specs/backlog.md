...
- [x] **El gate del step 4 pasó y ADR-041 quedó `accepted`** — corrido el 2026-09-15 contra el binario instalado desde el tag `v0.67.1`, nunca desde un worktree, y con el fix grepeado en site-packages antes de correrlo: el exit 0 del `uv tool install` no prueba nada, y el receipt de `uv` tenía `rev=v0.67.0` pinneado, así que un `--reinstall` pelado hubiera reinstalado la versión rota en silencio. El discriminador barato del fix es la **firma** `_log_block(decision, command, profile)`, que no puede existir sin el cambio de call site. Resultado: `PASS`, exit 0; el stdout quedó guardado como `run-0.67.1.log` al lado del script, porque el dir de la corrida (`run-f7gate178948476834223`) sólo tiene el estado generado y no preserva ni la salida ni el exit code. Hicieron falta tres corridas: la 1 falló la aserción C (#292, #294, #296); la 2, reacotada a la mitad de aislamiento, falló (#300); la 3 es el pase. **Siete defectos en las tres corridas, y no todos de la misma forma**: seis son de la forma «respuesta derivada del agente global donde la fuente es el agente del profile» —tres en #292, uno en #297 (el segundo de los dos readers que ADR-041 §3 pedía mover juntos, que quedó atrás un release) y dos en #300—; el séptimo es #294 y es el más filoso, `_planner_for` llamando `agent.name()` sobre un `@property`, con cuatro tests afirmando ese rechazo y los cuatro pasando porque los dos dobles declaraban `name()` como método. El gate **discrimina**: sale 1 contra 0.67.0 y también contra un shim que arregla sólo `pre-tool-use-security`. Lo que el pase **no** cubre, dicho para que no se lea de más: son **dos** builtins aserteados y no tres —`stop-verify-guard` es migrado pero no escribe `hooks.log`, su único sink es la MetricsDB acotada por `LH_DATA_DIR`— y los quince unmigrated se **cuentan**, no se fallan: siete filtraron 28 líneas en la corrida que pasó. El gate vive en `/tmp/f7-gate/`, fuera del repo, así que no es reproducible por CI — vale registrarlo, no se toca acá.
