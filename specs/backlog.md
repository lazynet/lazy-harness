...
### El gate F7 sale 1 contra el binario 0.67.2 instalado

**Medido el 2026-09-16**, corriendo `specs/gates/f7/isolation-gate.sh` con `F7_GATE_PYTHON=~/.local/share/uv/tools/lazy-harness/bin/python3` contra `lh 0.67.2` instalado —que es la versión de `main`—: **exit 1, 44 aserciones falladas**, 24 de la lane codex y 20 de la claude, todas con la misma forma (`did not reach <run>/profile-<lane>; found: (no entry written anywhere the gate watches)`). La sección 11 —«nada se movió fuera del config dir del profile invocado»— pasa **entera**, así que no es una fuga a otro profile: no se escribió nada en ningún lado. El bloque SCOPE es coherente: registry 18, asserted 11 (6 codex, 5 claude), skipped 7.

**No es regresión del cambio de prosa en `:247` ni de nada de esta pasada.** Las dos corridas —el script tal como está en `main` en `20caa21`, y el editado— son idénticas byte a byte salvo el token de la corrida. El defecto, si lo hay, precede a las dos, y es el motivo por el que se registra acá en vez de arreglarse de paso.

**Hipótesis, explícitamente no medida:** que **todas** las lanes fallen igual apunta antes a la config throwaway que el gate arma —¿habilita la escritura de `hooks.log` con la config que genera?— que a un routing roto, porque un routing roto haría fallar la sección 11, no la 10. Es una hipótesis y no un diagnóstico.

**Acción:** clasificarlo antes de tocar el script, con el dir de evidencia preservado y el `hooks.log` de la corrida leído a mano — no el exit code. Mientras no esté clasificado, una corrida verde de este gate no prueba nada.
