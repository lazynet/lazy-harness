...
**Shipped 2026-09-16** (#361). `agents/session_paths.py` ended up the one
importable answer for **ten** call sites, not the three `or "projects"` plus
one `or "logs"` this decision counted — six were literal reads that never went
through the adapter at all (`p.config_dir / "projects"` in three views,
`ingest.py:121`, and `logs`/`queue` in `views/_helpers.py`). `session_path`
returns `Path | None` and never falls back, exactly as specified. `lh doctor`
gained the **Transcripts** section this decision names. The metering question
this decision's brief also raised is a separate finding, recorded as
[ADR-051](../adrs/051-transcript-reader-does-not-meter.md) (superseded by
[ADR-053](../adrs/053-transcript-reader-carries-metering.md)).
...
**Shipped 2026-09-16** (#364, [ADR-050](../adrs/050-metric-event-v3-agent-and-billing-model.md)).
...
**Shipped 2026-09-16** ([ADR-043](../adrs/043-system-docs-by-role.md)). The
chezmoi source rename this decision's last paragraph describes is still
pending — see `../backlog.md` under *Multi-agente — items declarados*.
...
**Shipped 2026-09-16** (#366). `SEGMENT_FILES` is derived and tested against
the directory glob as specified; both renames landed with their old names kept
as aliases. The gate moved from `tool.native_name in INSPECTED_TOOLS` to
`tool.operation is Operation.MODIFY_FILE`, but the hardcoded
`matcher="Edit|Write"` was **dropped rather than derived**: no per-agent
matcher-derivation mechanism exists anywhere in this repo, so the real fix was
removing the literal and letting each adapter's own default take over — which
is also why the hook was inert on Codex, since `apply_patch` never matches
that literal.
...
**Shipped 2026-09-16** (#363, [ADR-049](../adrs/049-permission-bypass-intent.md)).
Codex's two measured positions, against `codex-cli 0.154.0`: `ENABLE` is `None`
(measured indistinguishable from baseline, not a gap) and `ACTIVATE` is
`--approve-for-me` (evidence §7). `--dangerously-bypass-hook-trust` stays a
separate axis and is never emitted, as required. `lcca` moves to
`lh run --bypass=enable` after the binary-first deploy — not in this PR.
...
**Measured 2026-09-16, failing, and open.** `write_envrc` does not carry one
export per agent claiming the root: `render_envrc` does a single `re.sub`
between `BEGIN_MARKER`/`END_MARKER` with no notion of "one export per agent",
so a second profile's `write_envrc` call over the same root **replaces** the
managed block instead of adding to it — the first profile's export disappears
silently. Reproduced with a throwaway script calling `write_envrc` twice over
one root with two agents; see `../backlog.md`, *Open — Prioridad MEDIA*, "D7 —
`write_envrc` sólo carga un export por root", for the measured before/after.
