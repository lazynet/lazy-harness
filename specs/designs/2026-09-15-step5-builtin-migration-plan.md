...
`tests/unit/test_hook_matcher_coverage.py` keys its subject list on `BuiltinHookSpec.operations` and holds each hook's deployed matcher against the `INSPECTED_TOOLS` its module publishes, so deleting the constant now fails that gate instead of quietly retiring the hook from it. It grepped module source for `tool_name` until 2026-09-15, which made **migration itself** — not deletion — the silent trigger: `main(event: HookEvent)` reads `event.tool.native_name`, the string disappears, and the hook drops out unwatched. `pre-tool-use-security` had already fallen out on `main` @ 75bedaa.
...
| 12 | `herdr-context-gauge` | **unset** — see below | — | `TOKEN_USAGE` **per placement** — see below | no | none |
...
**Row 12 is left unresolved on purpose, and task 12's first job is to resolve it.** The table asserts nothing it knows to be wrong.
  ...
  The signal set has no correct single value, which is why the row said so. `signals` applied to every placement of one spec, and this hook's placements disagree: the Stop path reads token usage, while the SessionEnd path deliberately reads no transcript at all (`:175`) because its whole job is retracting a dead session's gauge. Declaring `TOKEN_USAGE` makes `deploy` omit *both* on an adapter lacking the signal, leaving the gauge on the pane forever.

  **Resolved: modelled per placement.** `BuiltinHookSpec.signals` widened to `frozenset[Signal] | Mapping[str, frozenset[Signal]]` with a `signals_for(event)` accessor — the same widening `matcher` already carries, added for this same hook for this same reason. `post_tool_use`, `session_stop` and `session_start` declare `TOKEN_USAGE`; `session_end` is absent from the mapping and so declares nothing, and its retract installs everywhere. The split into a second builtin was rejected: `loader.py` maps one name to one module, so two specs cannot share one, and a second module would re-import the `HERDR_ENV` gate, the stamp and the subprocess wrapper while moving the stuck-gauge risk from deploy-time — where `_report_omitted` names what it dropped — to wiring-time, where nothing is printed.

  The cost the plan estimated for this option ("a change to the registry type — which reaches every hook and every reader of `signals`") was measured and is one production call site. `builtin_signals` is the only accessor, `signal_gaps.py:74` its only caller, and that call already sits inside a per-event loop; `deploy/engine.py:263` keys its omissions by `(event, hook)` and `_report_omitted` looks them up the same way. The pipeline was per-placement before this change and only the declaration was not. The audit is `tests/unit/hooks/test_signals_per_placement.py::test_the_registry_is_the_only_module_that_reads_the_signals_field`, executable rather than recorded here.

  Trap 2 applies here first, before either. Two named constants, `THROTTLED_EVENT` and `RETRACT_EVENT`, carry the canonical spellings; the branch that would otherwise compare against a wire name is covered by `tests/goldens/hooks/herdr-context-gauge/session-end-clears.json`'s companion argv assertion, which was watched turning `--clear-display-agent` into `--display-agent 🟢 94k` with the wire literals restored by hand.

- **Wave C** (tasks 9–12): the context-emitting hooks. `stop-context-rotate` imports `context_tokens` from `herdr_context_gauge` (`stop_context_rotate.py:39`), which was read as an ordering constraint and is not one: `ROTATE_TOKENS` (`:37`), `_format_tokens` (`:92`) and `context_tokens` (`:66`) are module-level constants and pure functions, all defined above `main()` and none touched by its signature, by a `signals` declaration or by a `migrated` flip. Task 12 moved no symbol, so task 11 may land first, second or concurrently; the two touch adjacent `_BUILTIN_HOOKS` entries and nothing else. Task 11 declares a plain `frozenset({TOKEN_USAGE})`, which the widened type still accepts.
