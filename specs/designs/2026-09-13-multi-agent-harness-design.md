...
| Codex's edit-adjacent hook payload names a native tool distinct from `Bash` (the `apply_patch` row of the provider table, `(source)`-tagged) | **Elevated to `(run)`, 2026-09-16** — six `codex exec` runs, model `gpt-6-astra`, in `specs/designs/codex-evidence.md`. Codex takes **two non-deterministic edit paths**, the model's own choice, not a version-gated switch: a `Bash` python-heredoc (`tool_name: "Bash"`, `tool_input.command` a shell script) in most runs, and a native patch tool in others — confirmed, once a *validated* deny envelope was used to force the hook to actually observe it, to fire `PreToolUse` with `tool_name: "apply_patch"` and `tool_input.command` carrying the raw patch text (`*** Begin Patch\n*** Update File: <path>\n@@\n-old\n+new\n*** End Patch`), the path embedded inside the blob rather than a structured field. An earlier revision of this row, based on a run whose deny envelope was malformed, read the native path as unhooked (`"file_change with no hook"`); that reading is **withdrawn** — the run in question fed Codex an invalid `permissionDecision`, which fails open per the row above, so the edit proceeding said nothing about whether the hook fired. A follow-up run with a validated envelope shows it does. |
...
| native tool names | `Bash`, `Read`, `Edit`, `Write` | Two non-deterministic edit paths, both **run, 0.154.0** (see the claims table above): `Bash` (python-heredoc edits, `tool_input.command` a shell script) and `apply_patch` (`tool_input.command` a raw patch-text blob, `*** Update File: <path>` embedded inside it, not a structured field) | `bash`, `view`, `rg`, `glob`, `task`, `skill`, `web_fetch` (**log, 1.0.83**) | unknown |
