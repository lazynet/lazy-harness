...
#       not blocked  -> it is hooks.json or trust. Arm B splits them.
#
#   B   identical, plus --dangerously-bypass-hook-trust        (only if A allowed)
...
# workspace. `hooks.json` and `config.toml` — the declaration and the trust store
# — are hashed before and after and reported if either moved. The hook log is
# appended to by the hooks themselves and never truncated: it is the deliverable,
# and probe 5 deleted its own.
#
...
TRUST_BEFORE="$(hash_of "$TRUST_STORE")"
...
if [ "$DECLARATION_BEFORE" = "$DECLARATION_AFTER" ] && [ "$TRUST_BEFORE" = "$TRUST_AFTER" ]; then
  ...
  echo "     config.toml  $TRUST_BEFORE -> $TRUST_AFTER" >&2
fi
