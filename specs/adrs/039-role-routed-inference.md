...
- The ADR-035 capability registry addresses this knob by a dotted string resolved with `getattr`, so moving the config gives **no compile-time signal**. Left behind, `lh doctor` and the config TUI would report the deprecated value and `toggle` would write to a dead field, with the suite green. The registry entry therefore moves in the same commit, guarded by a test asserting the registry and `run_inference` resolve the same active backend from one config.
