...
- `DEFAULT_HOOKS` stops being a literal maintained alongside `_BUILTIN_HOOKS` and becomes derived from the capability registry — `_derive_default_hooks()` walks `builtin_registry().capabilities(kind="hook")`, as the **Implemented** note above records — removing one instance of the "implemented but never wired" failure class. The registry carries fourteen of the loader's nineteen entries; the five it leaves out (`herdr-context-gauge`, `post-tool-use-ansible-lint`, `pre-tool-use-graph-assist`, `stop-verify-guard`, `user-prompt-goal`) are absent deliberately, with a reason given per hook in the note at the head of `plugins/builtins.py` — for graph-assist, that it ships under kill criteria and is removed by dropping it from `scripts`. That fourteen-against-nineteen gap is a documented choice, not drift.
