...
**Locate the file first, do not guess the path.** Run `lh memory status` and use the directory it reports — that command resolves it exactly the way every hook does. Do not reconstruct it: the resolution is `core/memory_store.py:memory_dir_for`, which puts memory in the knowledge store at `<store root>/<marker memory area>/<host>/<owner>/<repo>/`, keyed by the repository's normalised git remote rather than by the path of the checkout, and falls back to `<agent runtime dir>/<sessions subdir>/<encoded cwd>/memory/` only when there is no usable store or the checkout has no remote. Read `docs/how/memory-compound.md` for the full mechanics and an example of the exact JSON shape before writing anything — do not invent fields.
