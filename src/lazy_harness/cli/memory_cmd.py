...
# A repository contract is CLAUDE.md or, since ADR-060, AGENTS.md; a repo
# mid-migration can carry both, and both load.
_CONTRACT_NAMES = ("CLAUDE.md", "AGENTS.md")


def _find_contracts(root: Path, *, max_depth: int = _MAX_ROOT_SCAN_DEPTH) -> list[Path]:
    """Every contract (`CLAUDE.md`, `AGENTS.md`) reachable under `root`.

    Discovered by walking down to wherever a repository actually is rather than
    ...
    root (its contract, if any, is the whole contract — nothing inside it,
...
def _project_contracts(cfg: Config) -> list[tuple[str, Path]]:
    """`(label, path)` for every reachable contract under a configured root.
    ...
    for entry in cfg.profiles.items.values():
        for root in entry.roots:
            ...
            if not root_path.is_dir():
                continue
            for contract in _find_contracts(root_path):
                # Keyed on the file: roots of two profiles can overlap, and a
                # repo mid-migration carries two contracts that both load.
                if contract in seen_paths:
                    continue
                seen_paths.add(contract)
                found.append((f"project:{project_key(contract.parent)}", contract))
    return found


@memory.command("rightsize")
def rightsize() -> None:
    """List every contract the harness can reach and which ceiling it breaches.

    Read-only (ADR-030, Track 1b of the September 2026 harness improvements
    design). Covers profile contracts — each system doc the profile's agent
    loads (`CLAUDE.md` or `AGENTS.md`) — and the `CLAUDE.md`/`AGENTS.md` of
    every project reachable under a configured `[profiles.*].roots` entry,
    whether or not it has memory in the knowledge store yet. Thresholds are the same ones
    ...
    for profile in list_profiles(cfg):
        if not profile.exists:
            continue
        for doc in agent_for_profile(cfg, profile.name).system_docs():
            system_doc = profile.config_dir / doc
            if system_doc.is_file():
                entries.append((f"profile:{profile.name}", system_doc))

    entries.extend(_project_contracts(cfg))

    if not entries:
        click.echo("No CLAUDE.md or AGENTS.md files found.")
        return
