...
### 3. In-place upsert, never accumulate

`upsert_stats()` writes each `(session, model)` row with `ON CONFLICT … DO UPDATE`, replacing the stored totals with the freshly-recomputed ones. Re-ingesting the same session never adds to a running sum — it overwrites. Combined with (2), parsing a grown transcript yields the new cumulative total, not old+new. Sessions missing from a run keep their last-written row, so a transcript pruned by retention does not erase its cost from `lh status`.

## Pricing

`DEFAULT_PRICING` in `monitoring/pricing.py` holds per-million-token rates for the Claude models currently observed in the wild (`claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-4-8`, `claude-fable-5`, `claude-sonnet-5`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`). The rates mirror what LiteLLM publishes in `model_prices_and_context_window.json`, which is also what `ccusage` consumes — keeping the two aligned is the only way the cost numbers on `lh status` reconcile with `npx ccusage`.
...
- `test_ingest_preserves_sessions_after_transcript_pruned` — a session's row survives once its transcript is deleted by retention, so cost history does not shrink to a rolling window.
