...
1. Scope check — the hook only acts on `Edit` / `Write` tool calls, and only on two path shapes: a path ending in `/memory/MEMORY.md`, or one that is exactly `CLAUDE.md` / `AGENTS.md` or ends in `/CLAUDE.md` / `/AGENTS.md`. A near miss such as `NOTCLAUDE.md` is out of scope. Every other tool / path: instant exit 0. The tool gate is the *native* tool name rather than the normalised modify-file operation, because that operation also covers `NotebookEdit` and the filename match would not narrow it back out.
