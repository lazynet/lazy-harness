...
`lh memory proposals accept/reject/apply` also holds `<memory_dir>/.memory.lock` from reading the pending queue through archiving and replacing it, so CLI review cannot race the worker's updates.
