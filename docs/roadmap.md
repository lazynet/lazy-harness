...
- [x] **Run the contract gate against a throwaway second adapter.** Passed 2026-09-15, across 0.66.0 to 0.67.1, against the installed binary rather than a worktree. A throwaway `CodexAdapter` ran the migrated builtins against a disposable Codex profile: a hook fires, a blocking hook actually refuses, a hook whose required signal the second agent cannot supply is named and omitted rather than deployed, and each hook's log line lands in the profile that invoked it and nowhere else. Five production defects came out of the three runs it took to pass, all of one shape — an answer derived from the global agent where the profile's agent is the source. Two limits are recorded rather than papered over: `stop-verify-guard` is migrated but writes no log, so two builtins are asserted and not three; and the fifteen unmigrated builtins still leak their log line, counted on every run as a known gap and tracked to the next item.
