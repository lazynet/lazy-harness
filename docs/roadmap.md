...
- [x] **Run the contract gate against a throwaway second adapter.** Closed 2026-09-15, across 0.66.0 to 0.67.1, with the final run against the installed binary rather than a worktree. A throwaway `CodexAdapter` ran the migrated builtins against a disposable Codex profile, in three runs that each settled a different half. The first settled that a hook fires and that a blocking hook actually refuses, and failed on the third question — whether a hook whose required signal the second agent cannot supply is reported rather than deployed — which was answered by making the deploy omit and name it (0.67.0, the item above). The last run settled the half the first two could not see: that a hook invoked with `--profile <p>` writes its log line under that profile's directory and nowhere else. Six defects of one shape came out of the three runs — an answer derived from the global agent where the profile's agent is the source — plus one of another, a Protocol rejection path that never ran because `name` is a property and was being called as a method. Two limits are recorded rather than papered over: `stop-verify-guard` is migrated but writes no log, so the isolation half asserts two builtins and not three; and the fifteen unmigrated builtins still leak their log line, counted on every run as a known gap and tracked to the next item.
