...
- [x] **Skip — and name — hooks whose declared signals the profile's agent cannot deliver.** Released in 0.67.0. `BuiltinHookSpec.signals` had been inert: `stop-verify-guard` on an agent with no goal marker installed, ran, found nothing to verify and passed — green because it could not fail. The deploy now refuses those entries and names each omission in its output before it is an absence in the artifact, and `lh doctor` reports exactly what the deploy refuses, from the one rule in `hooks/signal_gaps.py`.
