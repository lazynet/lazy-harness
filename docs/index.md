...
- **Hooks.** A cross-platform hook engine with built-ins for session-start context injection and preflight checks, pre-compact summaries, session export, compound-loop enforcement, deterministic Engram mirroring, post-edit auto-format, and two `PreToolUse` gates — one for destructive commands, one for out-of-scope git operations. Bring your own hooks via config.
