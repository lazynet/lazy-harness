...
- **Hooks.** A cross-platform hook engine with built-ins for session-start context injection, pre-compact summaries, session export, compound-loop enforcement, deterministic Engram mirroring, post-edit auto-format, and a `PreToolUse` security gate. Bring your own hooks via config.
