...
- **Scheduler.** Declare recurring jobs in `config.toml` and `lh scheduler install` writes the native unit files. Today only the launchd (macOS) backend installs jobs — the systemd and cron backends are not implemented yet, so on Linux you still register jobs yourself.
