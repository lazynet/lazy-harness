...
| `version`                | string | `"0.9.67"`    | no       | Pinned Graphify version. `lh doctor` flags drift between this and the installed binary so upgrades are explicit. See [ADR-023](https://github.com/lazynet/lazy-harness/blob/main/specs/adrs/023-graphify-code-structure.md). |
...
| `max_pending_proposals`        | int            | `10`                          | no       | Pending `claude-md.proposal.md` rules above which the loop stops emitting new ones. While the queue is at the cap, proposals a session produces are **held back**, not queued: they are appended to `proposals-held.jsonl` next to the queue (one `{ts, rule, rationale, project}` record per line), which neither counts toward the cap nor reaches the grader as a rejection, and nothing moves them into the queue automatically. Emission resumes once the queue is drained with `lh memory proposals`, and `lh doctor` lists every halted queue on the machine. A queue nobody drains crowds the session-start context budget and teaches the grader nothing. |
| `backend`                      | string         | `"claude"`                    | no       | **Deprecated** — replaced by `[llm.roles].distill`. Still honoured when no `[llm]` table defines that role. The one-time deprecation warning fires only when this key or `model` is *declared* with a value other than its default: a config that leaves both alone never wrote the keys the notice asks it to migrate. See [`[llm]`](#llm). |
