...
| `max_pending_proposals`        | int            | `10`                          | no       | Pending `claude-md.proposal.md` rules above which the loop stops emitting new ones. While the queue is at the cap, proposals a session produces are **dropped**, not deferred; emission resumes once the queue is drained with `lh memory proposals`, and `lh doctor` lists every halted queue on the machine. A queue nobody drains crowds the session-start context budget and teaches the grader nothing. |
