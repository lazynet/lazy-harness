...
- **Engram persist** — its own section, reporting the Stop-hook loop's last-run age, failure rate over the runs considered, and cursor lag against the thresholds listed under Features. Prints `No runs yet` when the hook has never fired.
