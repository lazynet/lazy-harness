...
A built-in can also be declared for named agents only. `pre-tool-use-graph-assist`
runs on Claude Code alone, because Codex keeps graphify's own guard as the control
group of its measurement. Deploy leaves it out of every other agent's profile and
names that too:
...
`lh doctor` reads the same merged set, so it never reports that hook on those
profiles.

Detected MCP servers that a profile's adapter does not place are also named,
for example `· copilot/mcp: 3 detected servers not placed — adapter exposes no
...
The report is a sequence of sections. Most are silent when they have nothing to say, so a healthy machine prints far fewer than the full list below:

- **Environment checks** — Python version, agent binary present, config readable, profile dirs writable, `direnv` detected.
- **Features section** ([ADR-025](https://github.com/lazynet/lazy-harness/blob/main/specs/adrs/025-doctor-features-section.md)) — one row per memory-stack tool (`qmd`, `engram`, `graphify`) with state (`active`, `dormant`, `missing`), installed version vs pinned version, and a one-line hint when something needs attention. Tools that need an explicit enable in `config.toml` (e.g. `[memory.engram].enabled = true`) show as `dormant` until the flag flips. The `engram-persist` row reports loop health (`ok` / `warn` / `fail` / `missing`) classified from `~/.claude/logs/engram_persist_metrics.jsonl` against three thresholds: last-run age (warn ≥ 24 h, fail ≥ 7 d), recent failure rate (warn > 0%, fail > 10%), and cursor lag in bytes (warn when positive, fail ≥ 64 KiB unless the latest run hit its save cap, the persisted cursor advanced, and no save failed in that run).

- **Hook signals** — one row per deployed hook whose declared signals the profile's agent cannot deliver. Each built-in declares what it needs to read out of a transcript (messages, tool calls, token usage, an explicit goal); an adapter's `TranscriptReader` declares what it can actually deliver. The row names the difference, so a hook that installs cleanly and would then never fire is visible before it is trusted — `stop-verify-guard` on an agent that supplies no goal status is the case it exists for. Deployed hooks are resolved the same way `lh deploy` resolves them, not from a second copy of the defaults, and an adapter with no reader at all reports every declared signal as missing rather than erroring. An event the agent does not deliver *at all* is deliberately **not** reported here: nothing installs and nothing runs in that case, and it is the agent's event vocabulary that closes it rather than a transcript reader — collapsing the two would send you after the wrong mechanism. The section is omitted entirely when there is nothing to report.
- **Hook operations** — one row per deployed hook whose declared `Operation`s (decision 9) this profile's agent cannot fully produce. Each built-in declares which tool operations it reasons about (`run_command`, `read_file`, `modify_file`, `search_code`); an adapter's own native-tool map declares which of those its tools can actually emit. The row names the operation that never arrives — `pre-tool-use-security` losing `read_file` on Codex, whose tool map has no read tool at all — and says whether the hook is only partly blind or fully inert, the way `pre-tool-use-read-size` is on the same agent: it declares nothing else, so it installs, is asked about every tool call, and passes all of them. Closed by the agent's own tool map, never by this harness. The section is omitted entirely when there is nothing to report.
- **Transcripts** — one line per profile answering a single derived question: is this agent writing transcripts that nothing reads? The verdict comes from the adapter and the disk, never from a declaration — `<config_dir>/<declared sessions dir>` is globbed for `*.jsonl`, and `isinstance(adapter, TranscriptReader)` says whether a reader exists. Four outcomes: a reader with transcripts (`reads`), transcripts with no reader (`unread` — accurate for Copilot today, and a real regression the day it appears for Claude Code), a declared directory holding nothing (`no transcripts`, which also covers a declared directory that does not exist), and an agent that declares no sessions directory at all (`declares no sessions directory`). The absence of a location is decided **first**, and has to be: `Path(x) / ""` is `x`, so probing a config directory for an agent that declared nothing finds `settings.json` and reports every such agent as degraded. Nothing here fails `lh doctor` — a reader that has not been written yet is not a misconfiguration of this machine.
- **Hook events** — one row per deployed hook wired to an event this profile's agent does not carry at all. Distinct from a missing signal or a missing operation, both of which need the event delivered in the first place: here `hook_events()` has no key for it, so the hook never installs and never runs. The fix is the agent's own event vocabulary, never a `TranscriptReader` and never a tool-operation mapping. The section is omitted entirely when there is nothing to report.
- **MCP servers** — one row per profile whose adapter produces the same fresh config plan with or without the detected MCP servers, naming every server that is therefore not placed. The footer explains that the adapter exposes no MCP document the harness may write and the binary owns its own file. This is reporting only, never a failed health check, and the section is omitted when every adapter places its servers or no server is detected.
- **Profiles** — one line per configured profile with its agent and whether its directory exists. Always printed.
- **Shared roots** — one line per directory two or more profiles claim, naming the claimants and which of them is the default. Silent when nothing is shared, and it never fails `lh doctor`: an undeclared default is refused at launch, and this line exists so the refusal is visible before someone hits it.
- **Profile credentials** — one line per non-default profile with no `<name>.env` of its own in the secrets directory, which therefore launches under whatever credentials the ambient environment carries ([ADR-045](https://github.com/lazynet/lazy-harness/blob/main/specs/adrs/045-credential-boundary.md)). The line names no environment variable and prints nothing out of the file — an enumeration of which variables count as credentials would report "clean" for whichever one it missed. Silent when every profile has its own file.
- **Plugin registry** — one `✗` line per Claude Code plugin registry entry (`plugins/known_marketplaces.json`, `plugins/installed_plugins.json`) whose recorded path does not exist, which makes Claude Code skip that marketplace's plugins silently. When the same path exists inside the profile it says to run `lh deploy --profile <name>`, which repairs it; otherwise to reinstall the plugin. Any such line fails `lh doctor`.
- **Settings shape** — the deployed `settings.json` of every profile whose agent is Claude Code, run through the same detector `lh deploy` gates its writes with. One `✗` line per file carrying a key shaped like a hook declaration, naming the profile and the JSON path: Claude Code discards such a file whole — every hook, permission and `env` with it — and a profile nobody redeploys never meets the deploy-time refusal. The detector is a port of Claude Code 2.1.278's validator, so the line says so and shows what `--version` reports for the binary that profile launches — the one `lh run` resolves, which prefers Claude Code's version-manager directory over `claude` on `PATH` (`unknown` when no binary resolves or the probe fails); it makes no claim about other releases. A file that cannot be read, is not JSON, or is JSON but not an object gets a `!` line saying its shape cannot be checked, without failing. Any `✗` line fails `lh doctor`; silent when every file is clean or absent.
- **Knowledge dir** — a single present/missing line for `[knowledge].root`, printed only when one is configured.
- **Network egress** — one line per remote `[metrics]` sink, including a sink that is configured but inactive because the variable it names is unset or empty; `local-only` when none is configured. Only the endpoint's origin is printed, never the resolved URL.
- **LLM roles** — one row per role in `[llm.roles]`, each resolved and probed on its own so a broken entry is never reported by its healthy siblings ([ADR-039](https://github.com/lazynet/lazy-harness/blob/main/specs/adrs/039-role-routed-inference.md)). With no table declared it reports the deprecated single-backend form under `distill`, because printing nothing would read as "nothing configured".
- **Engram persist** — its own section, reporting the Stop-hook loop's last-run age, failure rate over the runs considered, and cursor lag against the thresholds listed under Features. A large lag with verified cursor progress at the save cap is a warning labelled `catching up`; a stalled cursor remains a failure. Prints `No runs yet` when the hook has never fired.
- **Memory hygiene** — project-memory drift for the resolved profile: `MEMORY.md` against its line and byte caps, and the pending proposal queue. Only an over-cap `MEMORY.md` fails the check; the rest is informational. Silent when the working directory has no project memory.
- **Halted proposal queues** — every `claude-md.proposal.md` on the machine, not just the current project's: each profile's declared sessions directory under `<encoded>/memory` and every project under the knowledge store. One `!` line per queue at or above `[compound_loop].max_pending_proposals` — pending count, how many proposals the cap has held back in `proposals-held.jsonl` when there are any, oldest date (`unknown` when no rule sits under a dated header — the producer counts those toward the cap all the same), path, and the `lh memory proposals list --memory-dir <path>` that drains it — then the number of halted queues and the proposals pending in them, and the total held back. Counted with the same parser `lh memory proposals list` numbers with, so archived blocks do not count. At the cap the compound loop stops recording new proposals for that project — a proposal a session produces while the queue is full is held back in `proposals-held.jsonl`, never queued automatically — and the session-start notice only reaches sessions opened in it; this section is where a queue in a project nobody opens shows up. A warning, never a failure: a halted producer degrades memory capture, it does not break the machine. Silent when no queue is at the cap.
- **Artifact versions** — one line per deployed artifact written by a *newer* lazy-harness than the installed one. Reporting, not refusing: failing here would turn a stale artifact into an unusable machine. Silent when nothing is newer.
- **Codex hook trust** — per Codex profile, how many deployed hooks are untrusted and how many carry a stored hash. Nothing is ever reported as *trusted*: telling trusted from modified means recomputing Codex's own hash, which would mean reimplementing its TOML normalisation and going silently wrong on any upstream change, so a stored hash reports as `unknown`. Never fails — untrusted is the expected state immediately after a deploy. Silent when no profile runs Codex, and when a Codex profile has no deployed `hooks.json`.
- **Launches** — non-Claude launches per profile over 28 days, and the blast-radius kill criterion's verdict once its horizon opens; before that it prints the date the horizon opens.

A **Sink freshness** section reports, for every `[metrics]` sink `plan_sinks()` currently resolves as active (everything except `sqlite_local`, which is the local store itself, not a delivery pipeline), how long it has been since anything was last enqueued in `sink_outbox` — `ok` / `warn` (≥ 24 h) / `fail` (≥ 7 d) / `missing` (no history yet, not the same as stale). This catches a sink that quietly stopped moving data — e.g. an environment variable that resolved when the config was written but is unset wherever the scheduled ingest job actually runs — instead of doctor passing because the sink is merely *configured*. The same section reports two further, independent signals over the rows still undelivered in `sink_outbox`: how many times the drain has failed on the worst of them — `warn` at 3 failed attempts, `fail` at 6 — and how long the oldest of them has gone undelivered, `warn` at 24 h and `fail` at 3 days. Enqueue age cannot see a broken endpoint, because a sink that refuses every request keeps accepting enqueues just fine. Attempt counts are the right unit for a refused sink, since a failed row is returned to the queue with a longer backoff and no attempt ceiling. But neither signal sees a queue that stalls while every POST succeeds — a re-enqueue that returned an already-delivered row to `pending` made the drain re-send the head of the queue on every run and never reach anything behind it, with enqueue age at minutes and attempts at zero. Age of the oldest undelivered row is what catches that, and it is safe against the machine-nobody-used case that rules age out for the other two, because an idle machine has nothing undelivered to age. The thresholds stay loose enough for a real backlog to clear: the drain moves one batch of 50 per ingest, so even a 2500-event burst is gone in about 13 hours. The verdict is the worst of the two, so neither failure mode can hide behind the other's green. This line is printed only once the drain has actually failed: between an enqueue and the drain that follows it there is always an untried row, so a backlog on its own is not a fault. The section is omitted entirely when `[monitoring].enabled` is false or no remote sink is active, since neither case has anything to check.

```bash
lh doctor
```

`lh doctor --json` emits one JSON object instead of the Rich text above, for a script or CI gate to read by key rather than parse text: `profiles`, `shared_roots`, `codex_trust`, `transcripts`, `launches`, `hook_signals`, `hook_operations`, `uncarried_events`, `mcp_gaps`. It is a deliberate subset of the text report — expensive sections that probe network or stateful filesystem data (egress, sink freshness, LLM backend reachability, memory hygiene, halted proposal queues, artifact versions, features, and settings shape, which runs the profile's Claude Code binary) are left out, while `mcp_gaps` reuses the local executable detection that deploy uses. This keeps `--json` fast and side-effect-free. The `codex_trust` verdicts are the same `CodexHookTrust` records the text section renders, so the two never disagree. Exit code and any error output share the same paths as the text form; only the successful case's stdout differs.
...
`--profile` names the profile the hook runs under, which decides its memory scope and its metrics label. It is optional: without it the profile is resolved from the agent's config-dir variable, which is how entries written before the flag existed keep working. A profile name no config declares — a rename or a redeploy the settings file has not caught up with — falls back to a declared profile running the same agent as the caller, with a warning on stderr, rather than refusing every tool call until the session restarts. The caller is read off the payload — Claude Code sends `prompt_id`, Codex sends `turn_id` — and the fallback is the configured default when it runs that agent, otherwise that agent's only declared profile. It never falls back across agents, because the fallback profile's adapter decides the wire format of the answer and the memory scope and metrics label it runs under — crossing agents would answer the caller in the wrong format under the wrong scope: when the caller cannot be identified, or no single profile of its agent exists, the hook still refuses.
...
```bash
lh memory status
lh memory status --memory-dir ~/repos/lazy/lazy-knowledge/memory/github.com/me/my-repo
```
...
```bash
lh memory consolidate
lh memory consolidate --memory-dir ~/repos/lazy/lazy-knowledge/memory/github.com/me/my-repo --last 100
```
...
```bash
lh memory reconcile
lh memory reconcile --memory-dir ~/repos/lazy/lazy-knowledge/memory/github.com/me/my-repo --check-contradictions
```
...
Lifecycle for the `claude-md.proposal.md` entries the compound loop appends (see [Memory: the compound loop](../how/memory-compound.md)). Every subcommand takes `--memory-dir <path>`; by default the per-project memory dir is resolved the way every other reader resolves it — into the knowledge store, keyed by the repository's git remote (`core/memory_store.py`), with the legacy per-cwd dir as the fallback. Inside a linked git worktree the key comes from the main checkout, so proposals stay on the file the loop writes to instead of following each worktree. `lh memory status` prints the resolved path.
...
Walks each profile's agent-declared sessions directory (`session_path(agent, config_dir, "sessions")`), aggregates token usage per `(session, model)`, prices it with `[monitoring.pricing]` overrides (falling back to `DEFAULT_PRICING`), and UPSERTs into `session_stats`. The pipeline is safe to run repeatedly — it tracks each session's file mtime in a separate `ingest_meta` table and skips files that haven't changed since the previous run. Re-ingesting the same file is idempotent: totals are re-computed from the full (append-only) JSONL and overwrite prior rows, so token counts never accumulate double.
...
lh metrics loops --db ~/.local/share/lazy-harness/metrics.db
# goal_absent          6
# goal_declared        3
# nontrivial_prompt    9
# session_closed       12
...
`lh profile move --from <a> --to <b>` relocates per-project conversation history between each profile's agent-declared sessions directories without losing JSONL history. Useful when reclassifying a project from one profile to another. Supports `--projects a,b,c`, `--all`, `--overwrite`, and `--yes`.
