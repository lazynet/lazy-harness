...
- **Halted proposal queues** — every `claude-md.proposal.md` on the machine, not just the current project's: each profile's `projects/<encoded>/memory` and every project under the knowledge store. One `!` line per queue at or above `[compound_loop].max_pending_proposals` — pending count, oldest date, path, and the `lh memory proposals list --memory-dir <path>` that drains it — then the number of halted queues and the proposals pending in them. Counted with the same parser `lh memory proposals list` numbers with, so archived blocks do not count. At the cap the compound loop stops recording new proposals for that project, and the session-start notice only reaches sessions opened in it; this section is where a queue in a project nobody opens shows up. A warning, never a failure: a halted producer degrades memory capture, it does not break the machine. Silent when no queue is at the cap.
