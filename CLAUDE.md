...
4. **`/tdd-check` passes before every commit**, all four checks, with pristine output. The fourth is `ruff format --check`; gating only on `ruff check` lets layout drift until it lands on top of a logic change.
