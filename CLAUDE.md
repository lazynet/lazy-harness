...
- **Every reader and writer of a config-derived path resolves it the same way.** Readers here honour `[monitoring] db` before falling back to the data dir; two hooks that skipped that lookup would have written a file nothing reads — zero forever, with no error. Grep all sides of a path before shipping a feature that spans components.
