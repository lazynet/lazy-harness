...
6. **Docs coherence is audited before every release.** Before a release-please release is cut, run `/coherence-audit` and verify the CLI reference matches implemented commands, hooks docs match registered hooks, every documented memory path matches `lh memory status` output, memory architecture docs match compound-loop artifacts, and the roadmap reflects completed ADR items. `uv run --group docs mkdocs build --strict` must pass.
...
- **Hooks handle every exception explicitly and exit 0.** Unhandled errors escape to the subprocess and crash the chain instead of degrading. The exception is a *blocking* hook, where exit 0 is itself the failure: exercise every exit-2 path with its dependencies mocked away and assert it still blocks. Critical-path exit codes are contractual.
...
- **A config field that promises automatic behaviour must have that behaviour implemented.** Grep for it at review time — `auto_rebuild_on_commit` shipped for months doing nothing. A docstring promising a deterministic operation — rebuild, reset, purge, atomic — is held to the same standard: delete known rows, re-invoke, and either it recovers them or the docstring says why it does not. An ingest docstring promising a rebuild left 2,139 orphaned rows behind for months.
- **Every path deriving a project key — memory or metrics — uses `git rev-parse --path-format=absolute --git-common-dir` and takes `.parent`**, never `--show-toplevel`. Worktrees otherwise fragment `decisions.jsonl` into directories nothing reads, and report one repository under as many project rows as it has branches. When fixing one, grep for all of them and test from both the main checkout and a worktree.
...
- **An implemented hook does not run until it is wired and its binaries are reachable.** Registration in `_BUILTIN_HOOKS` without a `config.toml [hooks.*]` entry passes every test and never executes. A binary in a peer project's virtualenv is invisible to a hook running from the installed tool — install it globally. A hook invoking a tool by bare command name resolves it from ambient `PATH`, never the `pyproject.toml` pin; the divergence is silent and surfaces as deterministic reformatting on edits, or as the flaky docs gate an old `uv` produced here.
- **A static list that should mirror a directory is derived from it, with a test asserting completeness against the glob.** `GUARDED_HOOKS` hand-maintained beside `hooks/builtins/*.py` drifts the moment a hook is added; a completeness test puts a new file in scope with no code change.
...
- **A pluggable protocol is audited across every code path that names its type or its config, not just the `Protocol` methods.** `AgentAdapter` gained Codex beside Claude and the leaks were all outside the adapter file — deploy, config, hook wiring, monitoring. Enumerate those call sites and check each against the new implementation before the PR lands.
...
- **A scheduled job is verified by running it through its scheduler, with its credentials resolved in that context.** `launchctl kickstart` or `systemctl start`, never code inspection and a file-existence check: the metrics sink reported inactive for 96 consecutive launchd runs while the same command worked from a terminal. Credentials belong in the job definition — plist `EnvironmentVariables`, systemd `EnvironmentFile` — never in shell init, which no scheduler reads; and the macOS Background domain denies Keychain, so a helper that silently falls back to it fails there and nowhere else.
...
- **A cost optimisation starts from a measurement of the suspect category, not an estimate.** Two token-cost hypotheses here — deferred tool schemas, a disconnected connector — were both disproved the moment they were instrumented. Diff what the first message loads against what the second one loads before spending effort on either.
