...
- **A test that passes with and without the thing it claims to cover, covers nothing.** Delete the guard, watch a test fail, restore by editing the file back by hand — never `git checkout`, which reverts the uncommitted implementation too. Anchor `pytest.raises(match=...)` on literal config keys or enum names, never a substring a `tmp_path` or traceback could also carry. Pair each explicit-parameter CLI test with a parameter-less smoke test. A double shaped around the buggy call site fails the same way silently, so exercise a Protocol's refusal path through the shipped sentinel (`NullAdapter`) and assert the error's structured attributes, not a substring of its message.
...
- **A claim about behaviour is verified by running the path, not by reading the name.** `--help` text, a docstring, a flag name and a config key prove a thing exists, never what it does — run the command and record the exit code, evaluate the expression, open the file. This binds design documents as tightly as code: an assertion about how an external binary behaves is held to the standard its own evidence section declares.
- **A hook that is registered is not a hook that runs.** Two silent narrowings sit between a builtin and a tool call. An explicit `[hooks.<event>].scripts` list *replaces* `DEFAULT_HOOKS[<event>]` wholesale (ADR-031), so a newly default-on hook never activates in a config that declares that event — grep the config for the hook's own name before claiming it is deployed. And a script that omits `matcher` inherits a per-event default (`pre_tool_use` → `Bash`), so a guard meant for `Read` or `Write` covers neither — declare the matcher and assert a test fails when it is narrowed. Prove it by firing the real operation, never by reading the registry.
- **A hook wired into shared config lands in every profile, including the ones missing what it needs.** `config.toml` is shared while identity is per profile, so a hook depending on a skill, command or config key blocks work in a profile that has none of them, with no recovery the user can find. Exercise it in every profile before wiring it, and deploy what it depends on in the same change.
- **Mutation testing proves a guard has branches, not that it has coverage.** For a denylist guard — block-if-match — a killed mutant says only that some rule fired. Attack the rule set with realistic evasions of its own patterns (whitespace, flag reordering, alternate path spellings) and record what got through: a denylist that has never been attacked has never been measured.
